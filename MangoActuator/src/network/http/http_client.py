...
class HttpClientApi(HttpBase):
    ...
    @classmethod
    async def download_file(cls, file_name):
    ...
    @classmethod
    async def upload_file(cls, file_path: str, file_name: str):
        data = {
        ...
        for i in range(3):
            ...
            response = await cls.post('/system/file', headers=headers, data=data, files=files)
            if response.code == 200:
                return response.data
            else:
                log.error(f'上传文件报错，请管理员检查，响应结果：{response.model_dump()}')
                await cls.login(SetConfig.get_username(), SetConfig.get_password())  # type: ignore

    @classmethod
    async def login(cls, username: str = None, password=None, retry=0, is_retry=False):
        try:
            response = await cls.post('/login', data={
                'username': username,
            ...
            if is_retry and retry < 100:
                ...
                return await cls.login(username, password, retry + 1, is_retry)
            return response
    ...
    @classmethod
    async def user_register(cls, json_data: dict):
        return await cls.post('/register', json=json_data)

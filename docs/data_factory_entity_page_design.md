...
```text
产品 -> 逻辑数据源 -> 表名 -> 实体名称 -> 主键字段 -> 唯一字段 -> 清理顺序
```

选择表名后，页面会读取表结构，并优先使用数据库表注释作为实体名称：
...
| POST | `/data-factory/entity/batch-generate` | 批量生成实体和字段规则，保留设计 |
...
```text
docs/data_factory_entity_batch_generate_design.md
```

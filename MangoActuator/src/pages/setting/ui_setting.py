...
class UiSettingPage(QWidget):
    def __init__(self, parent):
        ...
        self.combo_box_3 = MangoComboBox('请选择浏览器的H5模式，为空等于不开启', combo_box_3_list,
                                         SetConfig.get_web_h5(), is_form=False)  # type: ignore
        self.combo_box_3.click.connect(SetConfig.set_web_h5)  # type: ignore
        card_layout1.addRow('浏览器H5模式：', self.combo_box_3)
        self.toggle1 = MangoToggle(SetConfig.get_web_max())  # type: ignore
        self.toggle1.clicked.connect(SetConfig.set_web_max)  # type: ignore
        card_layout1.addRow('最大化：', self.toggle1)
        self.toggle2 = MangoToggle(SetConfig.get_web_recording())  # type: ignore
        ...
        self.label_2 = MangoLabel(','.join([f"{d['device_id']}({d['status']})" for d in self.get_adb_devices()]))

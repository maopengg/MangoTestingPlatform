<template>
  <TableBody ref="tableBody">
    <template #default>
      <a-card title="绑定模块目录" :bordered="false">
        <template #extra>
          <a-space>
...
<script lang="ts" setup>
  ...
  function onDataForm() {
    if (formItems.every((it) => (it.validator ? it.validator() : true))) {
      ...
      value['pytest_project'] = route.query.id
      if (data.isAdd) {
        postPytestModule(value)
          .then((res) => {
            Message.success(res.msg)
            doRefresh()
          })
  ...
  function doRefresh() {
    getPytestModule({
      pytest_project_id: route.query.id,
    })
      .then((res) => {
        data.data = res.data
      })

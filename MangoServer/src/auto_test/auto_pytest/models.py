...
class PytestProject(models.Model):
    ...
    def delete(self, *args, **kwargs):
        if PytestProjectModule.objects.filter(pytest_project=self).exists():
            raise ToolsError(300, "模块-有关联数据，请先删除绑定的数据后再删除！")
...
class PytestProjectModule(models.Model):
    ...
    def delete(self, *args, **kwargs):
        if PytestAct.objects.filter(module=self).exists():
            raise ToolsError(300, "过程对象-有关联数据，请先删除绑定的数据后再删除！")
        if PytestCase.objects.filter(module=self).exists():
            raise ToolsError(300, "测试用例-有关联数据，请先删除绑定的数据后再删除！")
        if PytestTools.objects.filter(module=self).exists():

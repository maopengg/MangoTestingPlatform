...
class CaseDistribution(WebRun):
    ...
    def __init__(self):
        super().__init__()
        self.web: Optional[NewChromium, NewWebkit, NewFirefox] = None
        # self.android: Optional[] = None

    def case_distribution(self, data: list[list[dict]] = data1):
        """
        分发用例给不同的驱动进行执行
        @param data: 用例列表
        @return:
    ...
    def web_test(self, case_obj: dict):
        if not self.web:
            self.__new_web_obj(case_obj['browser_type'], case_obj['browser_path'])
...
if __name__ == '__main__':
    ...
    r = CaseDistribution(equipment1, package1)
    r.case_distribution()

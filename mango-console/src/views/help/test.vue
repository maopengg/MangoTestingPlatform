    ...
    <div class="center">
      <div style="display: flex; flex-direction: column; height: 100%">
        <div class="center-data-item-wrapper">
          <DataItem class="item" title="界面自动化执行数" :data-model="testReportData.ui" />
          <DataItem class="item" title="接口自动化执行数" :data-model="testReportData.api" />
          <DataItem class="item" title="性能自动化执行数" :data-model="testReportData.perf" />
        </div>
...
import { testSuiteReport, testSuiteReportGetTypeSum } from '@/api/url'
...
const testReportData = reactive({
  ui: { num: 0, prefix: '', suffix: '', bg: Bg1 },
  api: { num: 0, prefix: '', suffix: '', bg: Bg2 },
  perf: { num: 0, prefix: '', suffix: '', bg: Bg3 }
})
...
function doGetTypeSum() {
  get({
    url: testSuiteReportGetTypeSum,
    data: () => {
      return {}
    }
  })
    .then((res) => {
      res.data.forEach((item: any) => {
        ...
        testReportData[key].num = item[key]
      })
    })
    .catch(console.log)
}

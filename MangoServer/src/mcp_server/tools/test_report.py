...
from src.auto_test.auto_system.views.test_suite import TestSuiteCRUD, TestSuiteSerializersC
from src.auto_test.auto_system.views.test_suite_details import (
...
from src.enums.tools_enum import EnvironmentEnum, TaskEnum, TestCaseTypeEnum
...
def _format_detail_item(item: dict) -> dict:
    ...
    if "result_data" in data:
        data["children"] = data.pop("result_data")
        if data.get("children"):
            for child in data["children"]:
                child["case_type"] = data["type"]
    return data
...
def _paged_details(
    ...
    return [_format_detail_item(item) for item in data_list], count

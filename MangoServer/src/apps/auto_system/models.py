...
class ProjectProduct(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        from src.apps.auto_api.models import ApiPublic, ApiCase, ApiInfo, ApiHeaders
        ...
        from src.apps.auto_ui.models import UiPublic, UiCase, PageSteps, Page
...
class ProductModule(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        if ApiCase.objects.filter(module=self).exists():
            raise ToolsError(300, "API用例-有关联数据，请先删除绑定的数据后再删除！")
        from src.apps.auto_ui.models import UiCase, PageSteps, Page
...
class TestObject(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        if DataFactoryDatasourceBinding.objects.filter(test_object=self).exists():
...
class TestSuiteDetailResult(models.Model):
    create_time = models.DateTimeField(verbose_name="创建时间", auto_now_add=True)
    update_time = models.DateTimeField(verbose_name="修改时间", auto_now=True)
    test_suite_detail = models.OneToOneField(
        to=TestSuiteDetails,
        to_field="id",
        related_name="result_payload",
        on_delete=models.CASCADE,
    )
    result_data = models.JSONField(verbose_name="完整测试结果", null=True)
    result_size = models.BigIntegerField(verbose_name="结果大小", default=0)

...
# @Author : 毛鹏
from auto_test.auto_system.service.test_suite.base_notice import NoticeMain
...
class UpdateTestSuite:
    ...
    def send_test_result(cls, test_suite_id: int, msg: str):
        ...
        if test_suite.is_notice != StatusEnum.SUCCESS.value and test_suite.tasks is not None and test_suite.tasks.notice_group and test_suite.tasks.is_notice == StatusEnum.SUCCESS.value:
            if (
                    test_suite.tasks.fail_notice == StatusEnum.SUCCESS.value and test_suite.status != StatusEnum.SUCCESS.value) or test_suite.tasks.fail_notice != StatusEnum.SUCCESS.value:

...
class TestObject(models.Model):
    ...
    name = models.CharField(verbose_name="被测试的对象", unique=True, max_length=64)
    value = models.CharField(verbose_name="被测试的对象", max_length=1024)
    # 0是web， 1是app， 2是小程序
    test_type = models.SmallIntegerField(verbose_name="对应什么客户端")
    executor_name = models.CharField(verbose_name="负责人", max_length=64, null=True)
...
class NoticeConfig(models.Model):
    """通知配置表"""
    team = models.ForeignKey(to=Project, to_field="name", on_delete=models.SET_NULL, null=True)
    name = models.CharField(verbose_name="通知名称", unique=True, max_length=64)

...
class ClientWebSocket:
    ...
    async def client_run(self):
        ...
        logger.info(str(f"websockets server url:{server_url}"))
        while True:
            try:
                ...
                notice_signal.send(1, data='已离线')
                logger.info("服务器已关闭，正在尝试重新链接，如长时间无响应请联系管理人员！")
            except OSError as error:
                notice_signal.send(1, data='已离线')
                logger.info(f"网络已中断，尝试重新连接中......{error}")
            except Exception as error:
                notice_signal.send(1, data=error)
                logger.info(f"socket发生未知错误：{error}")
    ...
    def __output_method(recv_json) -> SocketDataModel:
        ...
        try:
            ...
            logger.info(f'接收的消息提示:{out["msg"]}')
            if out['data']:
                logger.debug(f"接收的数据：{json.dumps(out['data'], ensure_ascii=False)}")
            return SocketDataModel(**out)
        except json.decoder.JSONDecodeError:
            logger.error(f'服务器发送的数据不可被序列化，请检查服务器发送的数据：{recv_json}')

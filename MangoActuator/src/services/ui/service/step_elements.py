...
class StepElements(ElementMain):
    steps_model: StepsModel = None
    page_step_result_model: PageStepsResultModel = None

    async def steps_init(self, steps_model: StepsModel):
        ...
        self.page_step_result_model = PageStepsResultModel(
            ...
            page_step_name=steps_model.name,
    ...
    async def steps_main(self) -> PageStepsResultModel:
        for element_model in self.steps_model.element_list:
            element_data = None
            if not self.is_step:
                for _element_data in self.steps_model.case_data:
            ...
            # 执行用例
            try:
                await self.element_setup(element_model, element_data, self.steps_model.type)
        ...
        if self.page and self.url:
            ...
            try:
                if data:
                    ...
                    self.driver_object.web_config = self.equipment_config
                    self.context, self.page = await self.driver_object.new_web_page()
                    await open_url()
            except TargetClosedError as error:
                await self.setup()

...
urlpatterns = [
    ...
    path("page/ass/method", PageStepsDetailedView.as_view({'get': 'get_ass_method'})),
    path("page/put/step/sort", PageStepsDetailedView.as_view({'put': 'put_step_sort'})),
    #
    path("element", PageElementCRUD.as_view()),
    path("element/name", PageElementViews.as_view({'get': 'get_element_name'})),
    path("element/put/is/iframe", PageElementViews.as_view({'put': 'put_is_iframe'})),
    path("element/test", PageElementViews.as_view({'post': 'test_element'})),
    #
    path("steps", PageStepsCRUD.as_view()),
    path("steps/put/type", PageStepsViews.as_view({'put': 'put_type'})),
    path("case/name", PageStepsViews.as_view({'get': 'get_case_name'})),
    path("steps/run", PageStepsViews.as_view({'get': 'ui_steps_run'})),
    path("steps/page/steps/name", PageStepsViews.as_view({'get': 'get_page_steps_name'})),
    path("copy/page/steps", PageStepsViews.as_view({'post': 'copy_page_steps'})),
    #

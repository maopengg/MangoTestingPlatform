...
class PageSteps:
    ...
    async def test_flow_data(self, flow_data: dict):
        async def search_element_model(_id) -> ElementModel:
            ...
            element_model = await self._get_element_data(element_model, flow_data.get('id'))
    ...
    async def _get_element_data(self, element_model, _id) -> ElementModel:
        if self.is_step:
            return element_model
        element_data = next(
            (_element_data
        ...
        if element_model.type == ElementOperationEnum.OPE or element_model.type == ElementOperationEnum.ASS:
            element_model.ope_value = [MethodModel(**i) for i in element_data.page_step_details_data]
        elif element_model.type == ElementOperationEnum.PYTHON_CODE:
            element_model.func = element_data.page_step_details_data
        elif element_model.type == ElementOperationEnum.SQL:
            element_model.sql_execute = element_data.page_step_details_data
        elif element_model.type == ElementOperationEnum.CUSTOM:
